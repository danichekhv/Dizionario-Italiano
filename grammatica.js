...
(function () {
  ...
  // Раздел темы в обе стороны — нужен lookupGrammar (app.js), чтобы собрать запрос к /api/grammar:
  // сам промпт (в т.ч. семь типов статей и требования к ним) теперь строит бэкенд, не клиент.
  function section(t) { const sec = sectionOf(t); return sec ? { it: sec.it, ru: sec.ru } : { it: '', ru: 'Grammatica' }; }
  ...
  function topicTile(t, withSection) {
    ...
    const gen = (!st && isAdmin())
      ? `<button class="gram-gen" onclick="event.stopPropagation();Grammatica.generate('${t.slug}')" title="Создать статью через бэкенд">создать</button>` : '';
  ...
  window.Grammatica = {
    ...
    // Для lookupGrammar: узнать каноническую тему по строке запроса; строку с подсказками
    // при вводе (список для выпадающего меню, как у словаря) даёт searchSuggest ниже
    topicByName(s) {
      const q = norm(s); if (!q) return null;
      return allTopics().find(t => t.slug === q || norm(t.it) === q || norm(t.ru) === q) || null;
    },
    // Подсказки при вводе в режиме grammar: до 8 совпадений по подстроке it/ru, для выпадающего списка
    searchSuggest(q) {
      const n = norm(q); if (!n) return [];
      return allTopics().filter(t => matches(t, n)).slice(0, 8);
    },
    relatedFor, section,
